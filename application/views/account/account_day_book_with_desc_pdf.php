        ...
        foreach ($row1 as $key => $row2) {
            foreach ($row2 as $key2 => $row_detail) {
                if($y_point > 190 || $y_point == 0) {
                    ...
                    $pdf->MultiCell(277, 10, U2T("หน้าที่ ".$page_index), 0, "R");

                    $pdf->SetFont('bold', '', 20 );
                    $pdf->SetXY( 0, $y_point );
                    $pdf->MultiCell(277, 10, U2T($_SESSION['COOP_NAME']), 0, "C");

                    $pdf->SetFont('bold', '', $font_size );
                    $y_point += 10;
                    $pdf->SetXY( 0, $y_point );
                    $pdf->MultiCell(277, 8, U2T("สมุดรายวันทั่วไป"), 0, "C");
                    ...
                    $pdf->MultiCell($col_w_7, $line_height, U2T("รายละเอียด"), 1, "C");
                    $y_point += $line_height;
                }
                ...
                $h = $pdf->GetY() > $h ? $pdf->GetY() : $h;
                $pdf->SetXY($col_x_7, $y_point);
                $pdf->MultiCell($col_w_7, $line_height, !empty($row_detail['description'])  ? U2T($row_detail['description']) : " ", 0, "L");
                $h = $pdf->GetY() > $h ? $pdf->GetY() : $h;
            ...
            $h = $pdf->GetY() > $h ? $pdf->GetY() : $h;
            $pdf->SetXY($col_x_7, $y_point);
            $pdf->MultiCell($col_w_7, $line_height, "", 0, "R");
            $h = $pdf->GetY() > $h ? $pdf->GetY() : $h;
            $group_credit_total = 0;
            $group_debit_total = 0;
            $row_height = $h - $y_point;
