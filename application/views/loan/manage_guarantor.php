...
<h1 style="margin-bottom: 0">การเปลี่ยนแปลงผู้ค้ำประกัน</h1>
<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 padding-l-r-0">
    ...
    <div class="col-xs-6 col-sm-6 col-md-6 col-lg-6 padding-l-r-0">
        <a class="btn btn-primary btn-lg bt-add" href="#" onclick="goto()">
			 พิมพ์รายงานเปลี่ยนแปลงผู้ค้ำประกัน
		</a>
    </div>
...
<div class="row gutter-xs">
    <div class="col-xs-12 col-md-12">
        <div class="panel panel-body">
            ...
            <form action="<?=base_url('loan/manage_guarantor')?>" method="post" name="myForm" id="myForm" onsubmit="event.preventDefault(); validateMyForm();">
                <div class="g24-col-sm-24" style="margin-bottom: 15px;">
                    <div class="form-group g24-col-sm-8">
                        <label class="g24-col-sm-10 control-label" >ประเภทสัญญา</label>
                        <div class="g24-col-sm-14">
                            <select name="loan_type_id" id="loan_type_id" class="form-control" onchange="selectScript()">
                                <option value="">ประเภทสัญญา</option>
                                <?php foreach ($loan_type as $key => $value){?>
                                    <option value="<?php echo $value['loan_name_id'];?>"> <?php echo $value['loan_name'];?></option>
                                <?php } ?>
                            </select>
                        </div>
                    </div>
                ...
                <div class="g24-col-sm-24" style="margin: 0px 15px 15px 0px;">
                    <div class="form-group g24-col-sm-16">
                        <div class="form-group g24-col-sm-5">
                            <h3 class="control-label" style="font-weight: 800;margin-top: 0px;margin-bottom: 0px;padding-top: 0px;">ผู้ค้ำประกันเดิม</h3>
                            
                        </div>
                        <div class="form-group g24-col-sm-18">
                            <label class="g24-col-sm-12 control-label"style='text-align: left; margin-bottom: 0px;  padding-top: 4px;'>อัปเดทวันที่ <span id="latest"></span></label>
                            <label class="g24-col-sm-12 control-label"style='text-align: left; margin-bottom: 0px;  padding-top: 4px;'>โดย <span id="updater"></span></label>
                        </div>
                    </div>
                </div>
                ...
                <div class="g24-col-sm-24" style="margin-bottom: 15px;">
                    ...
                    <div class="g24-col-sm-12" style="margin-left: 15px;">
                ...
                <div class="g24-col-sm-24" style="margin: 0px 15px 15px 0px;">
                    <div class="form-group g24-col-sm-16">
                        <h3 class="g24-col-sm-5 control-label" style="font-weight: 800;margin-top: 0px;margin-bottom: 0px;padding-top: 0px;">ผู้ค้ำประกันใหม่</h3>
                        <div class="g24-col-sm-18 control-label text-right" style="padding-right: 0px;">
                            <a href="#" class="btn btn-primary" onclick="add_guarantor();">เพิ่มผู้ค้ำใหม่</a>
                        </div>
                    </div>
                ...
                <div class="g24-col-sm-24" style="margin-bottom: 15px;">
                    ...
                    <div class="g24-col-sm-12" style="margin-left: 15px;">
                        <table class="table table-bordered table-striped table-center">
                            ...
                            <tbody id="new_guarantor">

                            </tbody>
                        </table>
                    </div>
                    <div class="form-group g24-col-sm-1"></div>
                    <div class="form-group g24-col-sm-2">
                        <table class="table table-bordered table-striped table-center">
                            ...
                            <tbody id="manage_guarantor"></tbody>
                        </table>
                    </div>
                </div>
...
<script>
            ...
            $.ajax({
                ...
                success:function(data) {
                    if(data=="FALSE"){
                        ...
                        $.each(row, (key, element) => {
                            if(typeof  element.contract_number !== "undefined"){
                                table += "<tr>";
                                table += "<td>"+element.contract_number+"</td>";
                                table += "<td>"+element.prename_short+element.firstname_th+" "+element.lastname_th+"</td>";
                                table += "<td><button class='btn btn-success' onclick=\"init('"+element.contract_number+"')\">เลือก</button></td>";
                                table += "</tr>";
                            }
                        });
                        table += "</tbody></table>";
    ...
    function runScript(e) {
        ...
        if(search === "" || loan_type_id === ""){
            return;
        }
        if (e.keyCode == 13) {
            blockUI();
            search_key(search, loan_type_id);
        }
    }
    ...
    function search_key(search, loan_type_id){
        if(search !== "" && loan_type_id !== ""){
            $.ajax({
                url: base_url+"ajax/search_loan_by_type",
                method:"post",
    ...
    function setData(obj){
        ...
        if(obj['latest'] != null){
            $("#latest").text(obj['latest']['create_date']);
            $("#updater").text(obj['latest']['user_name']);
        }else{
            $("#latest").text("-");
            $("#updater").text("-");
        }
    ...
    function add_guarantor(){
        ...
        table += "<td><input name='id_new_guarantor[]' type='text' class='form-control' id='id_new_guarantor_"+new_id+"' onkeypress=\"return getMember(event, this, '"+new_id+"')\"></td>";
        table += "<td><input name='name_guarantor[]' type='text' class='form-control' readonly id='fullname_new_guarantor_"+new_id+"' value=''></td>";
        table += "<td><input name='amount_new_guarantor[]' type='text' class='form-control is_numeral' onkeyup='format_number_1(event, this)' onchange='format_number_2(event, this)'></td>";
        table += "</tr>";
        $("#new_guarantor").append(table);

        var tableManage = "<tr style='background-color: white;height: 51px;' class='new_guarantor_"+new_id+"'>";
        tableManage += "<td colspan=2 style='vertical-align: bottom;'><span class='control-label' onclick=\"delete_guarantor('new_guarantor_"+new_id+"')\">ลบ</span></td>";
        tableManage += "</tr>";
        jQuery('#manage_guarantor').append(tableManage);
