<?php
	if(!empty($_GET['excel'])){
		...
		header("Content-Disposition: attachment; filename=สรุปรายการเรียกเก็บ แยกตามหน่วยงาน.xls");
...
<style>
	...
	<?php
		...
		$page_limit = empty(@$_GET['excel']) ? 20 : 9999;			
		$page_all = @ceil($num_rows/$page_limit);
		$total_data = array();
		$member_share_total = 0;
		$member_total = 0;
		
		$page_limit_first = empty(@$_GET['excel']) ? 18 : 9999;
		$page_link_limit = empty(@$_GET['excel']) ? 20 : 9999;
					...
					<tr>
						...
						?>
						 <td style="width:100px;vertical-align: top;" class="text-right">
						 	<a class="no_print"  target="_blank" href="<?php echo base_url('/report_processor_data/coop_report_charged_department_preview'.$get_param."&excel=1"); ?>">
						...
						if(!empty($rs)){
							...
							foreach(@$rs as $key => $row){
								...
								//if(@$_GET['dev'] == 'dev'){
									//echo $this->db->last_query(); echo '<hr>';
								//}
								// if($row['mem_group_name']=="กองจัดทำงบประมาณเพื่อการบูรณาการฯ" && @$_GET['dev']){
								// 	echo $this->db->last_query(); echo '<hr>';
								// }
								$array_data = array();
