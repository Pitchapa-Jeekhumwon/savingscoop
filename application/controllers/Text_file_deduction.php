...
class Text_file_deduction extends CI_Controller{
    ...
    public function index(){
        $arr_data = array();
        $this->libraries->template('text_file_deduction/index',$arr_data);
    }
    ...
    public function processor_index(){
        ...
        $this->libraries->template('text_file_deduction/processor_index',$arr_data);
    }
...
//            if($this->text_files->get_rows($data)){
            ...
            $get_data = $this->text_files->get_rows($data);
            if(!empty($get_data) && $get_data > 0){
//            if($this->text_files->get_rows($data)){
                $result['data_type'] = 'PROCESSOR';
                $result['has_data'] = 1;
            }else{
                $result['data_type'] = 'PROCESSOR';
                $result['has_data'] = 0;
    ...
    public function processor_data_txt(){
        if(isset($_GET['processor_mem_type']) && isset($_GET['processor_month']) && isset($_GET['processor_year'])) {
            $input = array();
            $input['month'] = $_GET['processor_month'];
            $input['year'] = $_GET['processor_year'];
            $input['mem_type_id'] = $_GET['processor_mem_type'];
            $input['mem_type_arr'] = $_GET['mem_type'];
            $data = $this->text_files->get_processor_data($input);
            $txt = "";
            if (sizeof($data)) {
                foreach ($data as $key => $item) {
                    $txt .= $this->text_files->processor_text_file($item);
//                    echo $this->text_files->processor_text_file($item);
                }
            }
            ...
            if($_GET['hun_mem_type'] == '1') {
                $this->text_files->create_file("COOP", $txt);
            }else{
                $this->text_files->create_file("COOP", $txt);
            }
        }
    ...
    public function sumary_processor(){
        $sumary = 0;
        $arr_data = array();
        if(isset($_POST['mem_type_id']) && isset($_POST['month']) && isset($_POST['year'])) {
            $input = array();
            ...
            $input['year'] = $_POST['year'];
            $input['mem_type_id'] = $_POST['mem_type_id'];
            $input['mem_type_arr'] = $_POST['mem_type_arr'];
            $data = $this->text_files->get_processor_month($input);
            if (sizeof($data)) {
                foreach ($data as $key => $item) {
                    $sumary+=$item['pay_amount'];
                }
